...
void loop() 
{
  ...
  if(prevSetTimerMode == 1 && setTimerMode == 0) //if the previous state was "set timer mode" and is currently in "count down mode"
  {
    ...
    blinkSwitchState=1; //Reset the built-in LED blinking animation back to the beginning
  ...
  {
    doneMode=false; //set the doneMode flag to false/0
    
    digitalWrite(13, LOW); //Set the built in LED to be off
  ...
  {
    if(timerCountDown<=0) //if we've counted all the way down
    {
      doneMode=true; //set the doneMode flag to true/1
      Serial.println("We're in doneMode!");

      //done mode animation
      doneModeStateMachine();
    }
    else //if we haven't counted all the way down yet
    {
      doneMode=false; //set the doneMode flag to false/0
...
void doneModeStateMachine() //Our state machine function
{
  switch(doneModeState) //The part that does the work, the switch statement
  {
    case 0: //Off and waiting
      //Do nothing
    ...
    case 1: //Off and transitioning to On
      //Start timer
      //Turn LED on
      doneModeTimer = millis(); //Start the timer
      digitalWrite(13, HIGH); //Turn the built in LED on
      digitalWrite(5, HIGH); //Turn LED 1 on
      digitalWrite(8, HIGH); //Turn LED 2 on
      digitalWrite(11, HIGH); //Turn LED 3 on
      doneModeState = 2; //go to case 2
      break;
    case 2: //On and waiting
      //Do nothing
    ...
    case 3: //On and transitioning to Off
      ...
      //Turn LED off
      doneModeTimer = millis(); //Start the timer
      digitalWrite(13, LOW); //Turn the built in LED on
      digitalWrite(5, LOW); //Turn LED 1 on
      digitalWrite(8, LOW); //Turn LED 2 on
      digitalWrite(11, LOW); //Turn LED 3 on
      doneModeState = 0; //go to case 0
      break;
  }
}
